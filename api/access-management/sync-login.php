...
try {
    $user = keycloak_admin_client()->ensureExternalApplicationAccess($userId);
    keycloak_security_store()->recordEvent(
        'external_user_login_access_synced',
        'info',
        $actor,
        keycloak_client_ip(),
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Akses login pengguna berhasil diaktifkan.',
        'data' => ['user' => $user],
    ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
} catch (KeycloakAdminException $exception) {
    keycloak_json_error(
        $exception->httpStatus(),
        'external_login_sync_failed',
        $exception->getMessage()
    );
} catch (Throwable $exception) {
    error_log('[biometric-security] external user login access sync failed');
    keycloak_json_error(
        500,
        'external_login_sync_failed',
        'Akses login pengguna belum berhasil diaktifkan.'
    );
}
